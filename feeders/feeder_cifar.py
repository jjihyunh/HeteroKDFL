...
class cifar:
    def __init__ (self, batch_size, num_workers, num_clients, num_classes, alpha, num_strongs):
        ...
        # Save pulbic datasets.
        self.public_partitions = {}
        if self.rank == 0:
            t=0
            for i in range (self.num_clients,self.num_clients+5*self.num_strongs,5):
                partition=[]
                for k in range(5):
                    name = "./partitions/cifar" + str(num_classes) + "_" + str(i+k) + ".txt"  
                    f = open(name, "r")
                    lines = f.readlines()
                    for j in range (len(lines)):
                        line = lines[j].split('\n')
                        value = int(line[0])
                        partition.append(value)
                self.public_partitions[t] = partition
                t=t+1
                f.close()
        
        self.partitions = self.comm.bcast(self.partitions, root = 0)
        self.public_partitions = self.comm.bcast(self.public_partitions, root = 0)
        if self.rank == 0:
            for i in range (self.num_clients):
                print ("worker " + str(i) + " has " + str(len(self.partitions[i])) + " samples")
            print("========================public==============================")
            for i in range (len(self.public_partitions)):
                print ("worker " + str(i) + " has " + str(len(self.public_partitions[i])) + " samples")
        ...
        self.num_local_public_samples = []
        for i in range (len(self.public_partitions)):
            self.num_local_public_samples.append(len(self.public_partitions[i]))
    ...
    def read_public_image (self, indices):
        info = indices.numpy()
        sample_id = info[0]
        device_id = info[1]
        index = self.public_partitions[device_id][sample_id]
        image = self.train_data[index]
        image = np.subtract(image, self.per_pixel_mean)
        image = np.divide(image, self.per_pixel_std)
        return image
    ...
    def augmentation(self, x, y):
        ...
        return x, y
    def public_augmentation(self, x):
        x = tf.image.pad_to_bounding_box(x, 4, 4, 40, 40)
        x = tf.image.random_crop(x, (32, 32, 3))
        x = tf.image.random_flip_left_right(x)
        return x
    ...
    def public_dataset (self, client_id):
        # Client_id should be the global client ID.
        num_samples = self.num_local_public_samples[client_id]
        client_index = np.full((num_samples, 1), client_id)
        sample_index = np.reshape(np.arange(num_samples), (num_samples, 1))
        indices = np.concatenate((sample_index, client_index), axis = 1)
        dataset = tf.data.Dataset.from_tensor_slices(indices)
        dataset = dataset.shuffle(num_samples, seed = int(time.time()))
        dataset = dataset.map(lambda x: tf.py_function(self.read_public_image, inp = [x], Tout = [tf.float32]))
        dataset = dataset.map(self.public_augmentation)
        dataset = dataset.batch(self.train_batch_size)
        dataset = dataset.repeat()
        return dataset.__iter__()
